...
public final class LinkedDequeThreadSafe<E extends Linked<E>> extends AbstractCollection<E> implements Deque<E> {
	 ...
	 */
	synchronized void linkFirst(final E e) {
		final E f = first;
		first = e;
	 ...
	 * @param e the unlinked element
	 */
	synchronized void linkLast(final E e) {
		final E l = last;
		last = e;
	...
	synchronized void checkNotEmpty() {
		if (isEmpty()) {
	...
	synchronized boolean contains(Linked<?> e) {
	 ...
	 */
	public synchronized void moveToFront(E e) {
		if (e != first) {
			unlink(e);
			linkFirst(e);
		}
	 ...
	 * @param e the linked element
	 */
	public synchronized void moveToBack(E e) {
		if (e != last) {
			unlink(e);
			linkLast(e);
		}
	...
	// A fast-path removal
	synchronized boolean remove(E e) {
		if (contains(e)) {
			unlink(e);
			return true;
		}
	...
	@Override
	  public synchronized Iterator<E> iterator() {
		Iterator<E> it = new AbstractLinkedIterator(first) {
	...
	abstract class AbstractLinkedIterator implements Iterator<E> {
		...
		/**
		 * Retrieves the next element to traverse to or <tt>null</tt> if there are no
		 * more elements.
		 */
...
interface Linked<T extends Linked<T>> {
	...
	/**
	 * Retrieves the next element or <tt>null</tt> if either the element is unlinked
	 * or the last element on the deque.
	 */
	T getNext();
